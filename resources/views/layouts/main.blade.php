...
<head>
    ...
    <meta name="description"
        content="@yield('meta_description', $settings->about_us ?? 'AKA Consulting menyediakan layanan konsultasi hukum, perizinan, dan manajemen kepatuhan untuk membantu bisnis beroperasi sesuai regulasi dan tumbuh berkelanjutan.')">
    <meta name="keywords"
        content="@yield('meta_keywords', 'konsultan hukum jakarta, jasa perizinan usaha jakarta, konsultan legalitas perusahaan, pendirian pt cv, konsultan hukum bisnis, jasa konsultan jakarta, konsultan kepatuhan bisnis, konsultan perizinan bandung, konsultan hukum surabaya, jasa legalitas usaha, sertifikasi halal bpom, konsultan tender, konsultasi hukum online, aka consulting, konsultan terpercaya indonesia')">
    ...
    <meta property="og:description"
        content="@yield('meta_description', $settings->about_us ?? 'AKA Consulting menyediakan layanan konsultasi hukum, perizinan, dan manajemen kepatuhan.')">
    <meta property="og:image"
        content="@yield('og_image', isset($settings) && $settings->logo_path ? asset('storage/' . $settings->logo_path) : asset('images/logo_aka.png'))">
    ...
    <script type="application/ld+json">
    {
      ...
      "priceRange": "$$",
      "telephone": "+{{ preg_replace('/^0/', '62', preg_replace('/[^0-9]/', '', $settings->phone ?? '')) }}",
...
<body class="site-shell selection:bg-primary selection:text-white">
            ...
            <div class="hidden items-center space-x-8 md:flex">
                ...
                @php
                    $navServices = \App\Models\Service::where('status', 'active')
                        ...
                        ->groupBy('category');
                @endphp
                <div class="relative" x-data="{ megaOpen: false }" @mouseenter="megaOpen = true"
                    ...
                    <div class="flex items-center gap-0.5">
                        ...
                        <button @click="megaOpen = !megaOpen" aria-label="Buka Menu Layanan"
                            ...
                            <span class="material-symbols-outlined text-[16px] transition-transform duration-200"
                                :class="megaOpen ? 'rotate-180' : ''">expand_more</span>
                        </button>
                    </div>
                    ...
                    <div x-show="megaOpen" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="mx-auto max-w-7xl px-4 md:px-8 lg:px-20 py-8">
                            <div class="flex items-center justify-between mb-6">
                                <div>
                                    <p class="text-[10px] font-black uppercase tracking-[0.3em] text-[#8d6408] mb-1">
                                        Layanan Kami</p>
                                    <h3 class="text-xl font-black text-slate-900">Pilih Layanan yang Anda Butuhkan</h3>
                                </div>
                                <a href="/layanan"
                                    class="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-widest text-[#8d6408] hover:underline">
                                    Lihat Semua
                            ...
                            @else
                                <div class="grid grid-cols-2 lg:grid-cols-{{ min($navServices->count(), 4) }} gap-6">
                                    @foreach($navServices as $catName => $catServices)
                                        <div>
                                            <p
                                                class="text-[9px] font-black uppercase tracking-[0.25em] text-[#8d6408] mb-3 flex items-center gap-1.5">
                                            ...
                                            <div class="flex flex-col gap-1">
                                                @foreach($catServices->take(5) as $svc)
                                                    <a href="{{ route('services.show', $svc->slug) }}"
                                                        ...
                                                        <span
                                                            class="material-symbols-outlined text-[18px] text-[#8d6408]/50 group-hover:text-[#8d6408] transition-colors shrink-0">{{ $svc->icon_image ?: 'gavel' }}</span>
                                                        <span
                                                            class="text-[12px] font-semibold text-slate-700 group-hover:text-[#8d6408] leading-tight transition-colors">{{ __t($svc->title) }}</span>
                                                    </a>
                                                @endforeach
                                                @if($catServices->count() > 5)
                                                    <a href="/layanan#{{ Str::slug($catName) }}"
                                                        class="text-[10px] font-black text-[#8d6408]/60 hover:text-[#8d6408] px-3 py-1.5 transition-colors">
                            ...
                            <div class="mt-6 pt-5 border-t border-[#f0e8d8] flex items-center gap-4">
                                <a href="/layanan"
                                    class="inline-flex items-center gap-2 rounded-full bg-[#8d6408] text-white px-5 py-2.5 text-[11px] font-black hover:bg-[#7a5a10] transition-colors">
                                    <span class="material-symbols-outlined text-[15px]">apps</span>
                                    Semua Layanan
                                </a>
                                <a href="{{ url('/#kontak') }}"
                                    class="inline-flex items-center gap-2 rounded-full border border-[#e0d0b0] bg-white text-[#8d6408] px-5 py-2.5 text-[11px] font-black hover:bg-[#f7f2e8] transition-colors">
            ...
            <div class="hidden items-center gap-4 md:flex">
                <div class="relative" x-data="{ langOpen: false }" @click.away="langOpen = false">
                    <button @click="langOpen = !langOpen"
                        class="flex items-center gap-2 border border-outline-variant/30 rounded-full px-3 py-1.5 bg-white/50 backdrop-blur-md text-xs font-bold text-on-surface hover:bg-surface-container-low transition-colors notranslate"
                        translate="no">
                        @if(app()->getLocale() == 'id')
                            <img src="https://flagcdn.com/w20/id.png" alt="Bendera Indonesia" width="20" height="15"
                                loading="lazy" class="w-4 h-3 object-cover rounded-[2px] shadow-sm">
                            <span>ID</span>
                        @else
                            <img src="https://flagcdn.com/w20/gb.png" alt="Bendera Inggris" width="20" height="15"
                                loading="lazy" class="w-4 h-3 object-cover rounded-[2px] shadow-sm">
                            <span>EN</span>
                        @endif
                        <span class="material-symbols-outlined text-[14px] transition-transform duration-200"
                            :class="langOpen ? 'rotate-180' : ''">expand_more</span>
                    </button>
                    <div x-show="langOpen" x-transition:enter="transition ease-out duration-200"
                        ...
                        x-transition:leave-start="opacity-100 translate-y-0"
                        x-transition:leave-end="opacity-0 translate-y-2"
                        class="absolute right-0 mt-2 w-36 bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.1)] border border-slate-100 py-2 z-50 notranslate"
                        style="display:none;" translate="no">
                        <a href="{{ route('lang.switch', 'id') }}"
                            class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors {{ app()->getLocale() == 'id' ? 'text-primary font-black bg-primary/5 border-l-2 border-primary' : 'text-slate-600 font-semibold border-l-2 border-transparent' }}">
                            <img src="https://flagcdn.com/w20/id.png" alt="Bendera Indonesia" width="20" height="15"
                                loading="lazy"
                                class="w-5 h-3.5 object-cover rounded-[3px] shadow-[0_1px_2px_rgba(0,0,0,0.1)]">
                            <span class="text-[11px] uppercase tracking-wider">Indonesia</span>
                        </a>
                        <a href="{{ route('lang.switch', 'en') }}"
                            class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors {{ app()->getLocale() == 'en' ? 'text-primary font-black bg-primary/5 border-l-2 border-primary' : 'text-slate-600 font-semibold border-l-2 border-transparent' }}">
                            <img src="https://flagcdn.com/w20/gb.png" alt="Bendera Inggris" width="20" height="15"
                                loading="lazy"
                                class="w-5 h-3.5 object-cover rounded-[3px] shadow-[0_1px_2px_rgba(0,0,0,0.1)]">
        ...
        <div data-mobile-nav-panel class="mobile-nav-panel hidden">
            ...
            <div class="mt-4 grid gap-2">
                <div class="px-4 mb-4" x-data="{ langOpenMob: false }">
                    <button @click="langOpenMob = !langOpenMob"
                        class="flex items-center justify-between w-full rounded-xl border border-outline-variant/30 px-4 py-3 bg-white/50 text-xs font-bold text-on-surface hover:bg-surface-container-low transition-colors notranslate"
                        translate="no">
                        ...
                        <span class="material-symbols-outlined text-[16px] transition-transform duration-200"
                            :class="langOpenMob ? 'rotate-180' : ''">expand_more</span>
                    </button>
                    <div x-show="langOpenMob" class="mt-2 flex flex-col gap-1 pl-2 notranslate" style="display:none;"
                        translate="no">
                        <a href="{{ route('lang.switch', 'id') }}"
                            class="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-slate-50 {{ app()->getLocale() == 'id' ? 'text-primary font-black bg-primary/5' : 'text-slate-600 font-semibold' }}">
                            <img src="https://flagcdn.com/w20/id.png" alt="ID"
                                class="w-5 h-3.5 object-cover rounded-[3px] shadow-sm">
                            <span class="text-xs">Indonesia</span>
                        </a>
                        <a href="{{ route('lang.switch', 'en') }}"
                            class="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-slate-50 {{ app()->getLocale() == 'en' ? 'text-primary font-black bg-primary/5' : 'text-slate-600 font-semibold' }}">
                            <img src="https://flagcdn.com/w20/gb.png" alt="EN"
                                class="w-5 h-3.5 object-cover rounded-[3px] shadow-sm">
                    ...
                    <div class="flex items-center justify-between rounded-xl hover:bg-surface-container-low group">
                        ...
                        <button @click="layananOpen = !layananOpen" aria-label="Buka Submenu Layanan"
                            ...
                            <span class="material-symbols-outlined text-[18px] transition-transform"
                                :class="layananOpen ? 'rotate-180' : ''">expand_more</span>
                        </button>
                    </div>
                    <div x-show="layananOpen" class="ml-4 mt-1 flex flex-col gap-1" style="display:none;">
                        @foreach($navServices as $catName => $catServices)
                            <p class="text-[9px] font-black uppercase tracking-widest text-[#8d6408] px-4 pt-3 pb-1">
                                {{ Str::limit($catName, 30) }}
                            </p>
                        ...
                        @endforeach
                        <a href="/layanan"
                            class="flex items-center gap-2 rounded-xl px-4 py-3 text-[11px] font-black text-[#8d6408] hover:bg-[#f7f2e8] mt-1">
    ...
    <footer class="w-full bg-[#0e0b07] text-white">
        ...
        <div class="border-b border-white/5 bg-[#13100a]">
            <div
                class="mx-auto max-w-7xl px-4 md:px-8 lg:px-20 py-8 flex flex-col md:flex-row items-center justify-between gap-5">
                <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.3em] text-[#d9a11a] mb-1">
                        {{ __('Konsultasi Gratis') }}
                    </p>
                    <p class="text-lg font-black text-white">
                        {{ __('Siap membantu kebutuhan legal & perizinan bisnis Anda') }}
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-16">
                ...
                <div class="md:col-span-4">
                    <a href="/" class="inline-flex items-center gap-3 mb-5 group">
                        ...
                        <span
                            class="text-lg font-black text-white tracking-tight">{{ $settings->company_name ?? 'AKA Consulting' }}</span>
                    </a>
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        <span
                            class="inline-flex items-center gap-1.5 rounded-full border border-[#d9a11a]/25 bg-[#d9a11a]/8 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-[#d9a11a]">
                            <span class="material-symbols-outlined text-[12px]">verified</span>
                            Legal Certified
                        </span>
                        <span
                            class="inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-white/4 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-white/35">
                    ...
                    @if(isset($settings) && ($settings->address ?? null))
                        <div class="flex items-start gap-2.5 text-white/35 text-xs leading-relaxed mb-5">
                            <span
                                class="material-symbols-outlined text-[#d9a11a] mt-0.5 text-sm shrink-0">location_on</span>
                    ...
                    {{-- Contact Admin WA --}}
                    <div>
                        <p class="text-[9px] font-black uppercase tracking-[0.28em] text-white/20 mb-3">
                            {{ __('Hubungi Admin via WhatsApp') }}
                        </p>
                        <div class="flex flex-col gap-2">
                            @php
                                $footerAdmins = $settings->whatsapp_admins ?? [];
                            @endphp
                            @if(!empty($footerAdmins))
                                @foreach($footerAdmins as $adm)
                                    @php
                                        $admPhone = preg_replace('/[^0-9]/', '', $adm['phone'] ?? '');
                                        if (str_starts_with($admPhone, '0')) $admPhone = '62' . substr($admPhone, 1);
                                        $waText = $settings->whatsapp_text ?? 'Halo ' . ($adm['name'] ?? '') . ', saya ingin konsultasi layanan AKA Consulting.';
                                    @endphp
                                    <a href="https://wa.me/{{ $admPhone }}?text={{ urlencode(__($waText)) }}"
                                        target="_blank" rel="noopener noreferrer"
                                        ...
                                        <div
                                            class="w-7 h-7 rounded-full bg-[#25D366]/10 flex items-center justify-center shrink-0 group-hover:bg-[#25D366]/20 transition-colors">
                                        ...
                                        <div>
                                            <p
                                                class="text-[11px] font-black text-white/65 group-hover:text-white transition-colors">
                                                {{ $adm['name'] ?? 'Admin' }}
                                            </p>
                                            <p class="text-[10px] text-white/25 font-mono">{{ $adm['phone'] ?? '' }}</p>
                                        </div>
                                        <span
                                            class="material-symbols-outlined text-[14px] text-white/15 group-hover:text-[#25D366] transition-colors ml-auto">arrow_forward</span>
                                    </a>
                                @endforeach
                            @endif
                        </div>
                ...
                <div class="md:col-span-3">
                    <div class="flex items-center gap-2 mb-5">
                        ...
                        <h4 class="text-[9px] font-black uppercase tracking-[0.3em] text-[#d9a11a]">{{ __('Layanan') }}
                        </h4>
                    </div>
                    <ul class="space-y-2.5">
                        ...
                        @forelse($footerServices as $fs)
                            <li>
                                <a href="/layanan/{{ $fs->slug }}"
                                    ...
                                    <span
                                        class="w-1 h-1 rounded-full bg-[#d9a11a]/30 group-hover:bg-[#d9a11a] transition-colors shrink-0"></span>
                        ...
                        @empty
                            <li>
                                <a href="/layanan"
                                    class="text-white/35 hover:text-white transition-colors text-sm">{{ __('Lihat Semua Layanan') }}</a>
                            </li>
                        @endforelse
                        <li class="pt-1">
                            <a href="/layanan"
                                class="inline-flex items-center gap-1.5 text-[11px] font-black text-[#d9a11a] hover:text-[#f0c930] transition-colors">
                                {{ __('Lihat Semua') }}
                ...
                <div class="md:col-span-4">
                    <div class="flex items-center gap-2 mb-5">
                        <span class="w-4 h-px bg-[#d9a11a]"></span>
                        <h4 class="text-[9px] font-black uppercase tracking-[0.3em] text-[#d9a11a]">
                            {{ __('Perusahaan') }}
                        </h4>
                    </div>
                    <ul class="space-y-2.5 mb-10">
                        <li><a href="/tentang-kami"
                                class="group flex items-center gap-2 text-white/35 hover:text-white transition-colors text-sm"><span
                                    class="w-1 h-1 rounded-full bg-[#d9a11a]/30 group-hover:bg-[#d9a11a] transition-colors shrink-0"></span>{{ __('Tentang Kami') }}</a>
                        </li>
                        <li><a href="/portofolio"
                                class="group flex items-center gap-2 text-white/35 hover:text-white transition-colors text-sm"><span
                                    class="w-1 h-1 rounded-full bg-[#d9a11a]/30 group-hover:bg-[#d9a11a] transition-colors shrink-0"></span>{{ __('Portofolio') }}</a>
                        </li>
                        <li><a href="/blog"
                                class="group flex items-center gap-2 text-white/35 hover:text-white transition-colors text-sm"><span
                                    class="w-1 h-1 rounded-full bg-[#d9a11a]/30 group-hover:bg-[#d9a11a] transition-colors shrink-0"></span>{{ __('Blog & Artikel') }}</a>
                        </li>
                        <li><a href="{{ url('/#kontak') }}"
                                class="group flex items-center gap-2 text-white/35 hover:text-white transition-colors text-sm"><span
                                    class="w-1 h-1 rounded-full bg-[#d9a11a]/30 group-hover:bg-[#d9a11a] transition-colors shrink-0"></span>{{ __('Kontak Kami') }}</a>
                        </li>
                    </ul>
        ...
        <div class="border-t border-white/5 py-8">
            <div class="mx-auto max-w-7xl px-4 md:px-8 lg:px-20">
                <p class="text-[9px] font-black uppercase tracking-[0.3em] text-[#d9a11a]/60 mb-4">
                    {{ __('Wilayah Layanan') }}
                </p>
        ...
        <div class="border-t border-white/5">
            <div
                class="mx-auto max-w-7xl px-4 md:px-8 lg:px-20 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
                <p class="text-white/20 text-xs">
                    &copy; {{ date('Y') }} <span
                        class="text-white/35 font-semibold">{{ $settings->company_name ?? 'AKA Consulting' }}</span>.
                    {{ __('Hak Cipta Dilindungi') }}.
                </p>
                <div class="flex items-center gap-3 flex-wrap justify-center md:justify-end">
                    <a href="{{ route('privacy') }}"
                        class="text-white/20 text-[10px] hover:text-white/45 transition-colors">{{ __('Kebijakan Privasi') }}</a>
                    <span class="w-px h-3 bg-white/10"></span>
                    <a href="{{ route('terms') }}"
                        class="text-white/20 text-[10px] hover:text-white/45 transition-colors">{{ __('Syarat & Ketentuan') }}</a>
                            ...
                            @foreach($footerSocials as $social)
                                @if(!empty($social['url']))
                                    <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer"
                                        ...
                                        <svg class="w-3 h-3 text-white/25 group-hover:text-[#d9a11a] transition-colors"
                                            fill="currentColor" viewBox="0 0 24 24">{!! $social['icon'] !!}</svg>
                                    </a>
                                @endif
                            @endforeach
                        </div>
    ...
    <div x-data="{ 
         ...
         }" x-show="showWidget" x-transition:enter="transition ease-out duration-500"
        ...
        <div x-show="showTooltip && !open" x-transition:enter="transition ease-out duration-500"
            ...
            <div class="flex items-center gap-2 mb-1.5">
                <span
                    class="w-2.5 h-2.5 bg-primary rounded-full animate-pulse shadow-[0_0_8px_rgba(0,159,227,0.5)]"></span>
                <span class="text-[9px] font-black text-primary uppercase tracking-widest">Online Support</span>
            </div>
            <p class="text-[11px] text-slate-700 leading-relaxed font-semibold">Halo! Ada yang bisa kami bantu hari ini?
            </p>
            <div class="absolute bottom-[-6px] right-6 w-3 h-3 bg-white rotate-45 border-r border-b border-slate-100">
            </div>
        </div>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="p-6 border-b border-slate-50 bg-white">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] font-black text-primary uppercase tracking-[0.2em]">Online
                            Support</span>
                    </div>
                    <button @click="open = false" aria-label="Tutup Panel Chat"
            ...
            <div class="p-6 bg-slate-50/20">
                <form action="{{ route('konsultasi.store') }}" method="POST" class="space-y-3.5">
                    ...
                    <input type="text" name="_website_url" class="hidden" style="display:none" tabindex="-1"
                        autocomplete="off">
                    ...
                    <button type="submit"
                        class="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl shadow-lg hover:bg-primary transition-all flex items-center justify-center gap-2 group">
                        <span class="text-sm">Kirim ke Admin</span>
                        <span
                            class="material-symbols-outlined text-sm group-hover:translate-x-0.5 transition-transform notranslate"
                            translate="no">send</span>
                    </button>
                </form>

                <div class="mt-6 flex flex-col gap-3" x-data="{ showAdmins: false }">
                    <p class="text-[9px] text-slate-400 font-black uppercase tracking-[0.2em] text-center">Atau Hubungi
                        Langsung</p>
                    ...
                    <button @click="showAdmins = !showAdmins"
                        ...
                        <span class="material-symbols-outlined text-[16px] transition-transform duration-300"
                            :class="showAdmins ? 'rotate-180' : ''">expand_more</span>
                    </button>

                    {{-- Pilihan Admin --}}
                    <div x-show="showAdmins" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 -translate-y-2"
                        x-transition:enter-end="opacity-100 translate-y-0" class="flex flex-col gap-2">

                        @php
                            $chatAdmins = $settings->whatsapp_admins ?? [];
                        @endphp
                        @if(!empty($chatAdmins))
                            @foreach($chatAdmins as $adm)
                                @php
                                    $admPhone = preg_replace('/[^0-9]/', '', $adm['phone'] ?? '');
                                    if (str_starts_with($admPhone, '0')) $admPhone = '62' . substr($admPhone, 1);
                                    $waText = $settings->whatsapp_text ?? 'Halo ' . ($adm['name'] ?? '') . ', saya ingin konsultasi layanan AKA Consulting.';
                                ...
                                <a href="https://wa.me/{{ $admPhone }}?text={{ urlencode(__($waText)) }}"
                                    ...
                                    <div
                                        class="w-9 h-9 rounded-full bg-[#25D366]/15 flex items-center justify-center shrink-0 group-hover:bg-[#25D366]/25 transition-colors">
                                        <span class="material-symbols-outlined text-[18px] text-[#25D366]">support_agent</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-[12px] font-black text-slate-900">{{ $adm['name'] ?? 'Admin' }}</p>
                                        <p class="text-[10px] text-slate-400">{{ $adm['phone'] ?? '' }}</p>
                                    </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($activeChatSocials as $cs)
                                    <a href="{{ $cs['url'] }}" target="_blank" rel="noopener noreferrer"
                                        ...
                                        aria-label="{{ $cs['label'] }}">
                                        <svg class="w-3 h-3 shrink-0" fill="{{ $cs['color'] }}"
                                            viewBox="0 0 24 24">{!! $cs['icon'] !!}</svg>
                                        {{ $cs['label'] }}
                                    </a>
                                @endforeach
                            </div>
        ...
        <button @click="open = !open; showTooltip = false" aria-label="Buka Chat Layanan"
            ...
            <span class="material-symbols-outlined text-2xl relative z-10 notranslate" translate="no"
                x-show="!open">chat</span>
            <span class="material-symbols-outlined text-2xl relative z-10 notranslate" translate="no" x-show="open"
                style="display: none;">close</span>
        </button>
    </div>

    <!-- Cookie Consent Banner -->
    <div x-data="{ showCookie: !localStorage.getItem('cookieConsent') }" x-show="showCookie"
        ...
        class="fixed bottom-0 left-0 w-full z-50 bg-white border-t border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] p-4 md:p-6"
        style="display: none;">
        <div class="mx-auto max-w-7xl flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex-1">
                <p class="text-sm text-slate-600 leading-relaxed">
                    Kami menggunakan cookies untuk meningkatkan pengalaman pengguna, menganalisis lalu lintas, dan
                    menyajikan konten yang relevan. Dengan melanjutkan penggunaan situs ini, Anda menyetujui <a
                        href="{{ route('privacy') }}" class="text-primary font-semibold hover:underline">Kebijakan
                        Privasi</a> kami.
                </p>
            </div>
            <div class="flex items-center gap-3 shrink-0">
                <button @click="showCookie = false"
                    class="px-5 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-100 rounded-lg transition-colors">Tolak</button>
                <button @click="localStorage.setItem('cookieConsent', 'true'); showCookie = false"
                    class="btn-primary shadow-sm px-6 py-2">Terima Cookies</button>
            </div>
        </div>
    </div>

    {{-- ═══ BACK TO TOP BUTTON (Global) ═══ --}}
    <button id="back-to-top" aria-label="Kembali ke atas" onclick="window.scrollTo({ top: 0, behavior: 'smooth' })"
